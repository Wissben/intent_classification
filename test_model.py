...
def test(texts=None):
    ...
    for text in texts:
        ...
        word_postags_encoded = [config.POS2OH[w] for w in postags]
        ...
        tags_decoded = [config.INDEX2TAG[np.argmax(pred)] for pred in tags]
        intent_decoded = [config.INDEX2INTENT[np.argmax(intent[0])]]
        print(text)
